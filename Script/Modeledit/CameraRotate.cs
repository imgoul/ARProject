...
public class CameraRotate : MonoBehaviour {
    ...
    void ScrollView()//滚轮操作
    {
        if (Carbon.atom != null)
        {
            ...
            distance = Vector3.Distance(transform.position, Carbon.atom.transform.position);
            distance = Mathf.Clamp(distance, 0.9f, 6);//限定距离最小及最大值

            if (Input.GetAxis("Mouse ScrollWheel") > 0 && distance > 0.95f)
            {
                Vector3 vector = transform.position - (transform.position - Carbon.atom.transform.position).normalized * scrollSpeed;
                transform.position = vector;

            }
            if (Input.GetAxis("Mouse ScrollWheel") < 0 && distance < 12f)
            {
                Vector3 vector = transform.position + (transform.position - Carbon.atom.transform.position).normalized * scrollSpeed;
                transform.position = vector;
    ...
    void rotateview()//上下左右移动视野
    {
        if (Carbon.atom != null)
        {
            Transform prefabposion = Carbon.atom.transform;
